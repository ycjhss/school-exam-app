...
const RatioRow = ({ item, year, sem, grade, onSave, onDelete }) => {
  ...
  const performanceFields = ['perf1', 'perf2', 'perf3', 'perf4', 'perf5'];
  ...
  const handleChange = (field, val) => {
    setFormData(prev => {
      ...
      if (field === 'isPerformanceNotConducted' && val) {
        performanceFields.forEach(performanceField => { newData[performanceField] = ''; });
      }
      if (['exam1', 'exam2', ...performanceFields, 'isPerformanceNotConducted'].includes(field)) {
        let baseSum = 0; let essaySum = 0; let hasNumber = false;
        ['exam1', 'exam2', ...performanceFields].forEach(f => {
  ...
  const performanceNotConducted = Boolean(formData.isPerformanceNotConducted);
  const renderPerformanceCell = (field) => (
    <td className={`border border-gray-300 p-0 ${performanceNotConducted ? 'bg-gray-100' : ''}`}>
      {performanceNotConducted ? (
        <div className="w-full text-center p-1.5 text-[11px] font-bold text-gray-400">미실시</div>
      ) : isConfirmed ? (
        <div className="w-full text-center p-1.5 text-[11px] text-gray-600 bg-gray-50">{String(formData[field] || '')}</div>
      ) : (
        <input type="text" value={String(formData[field] || '')} onChange={e=>handleChange(field, e.target.value)} className="w-full text-center p-1.5 text-[11px] outline-none focus:bg-amber-50 bg-transparent"/>
      )}
    </td>
  );
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors group">
      <td className="border border-gray-300 p-0 relative">
        {isConfirmed ? (
          <div className="w-full text-center p-1.5 text-[11px] font-bold text-gray-700 bg-gray-50">
            <div>{String(formData.subject || '')}</div>
            {performanceNotConducted && <div className="mt-1 text-[9px] text-gray-400">수행평가 미실시</div>}
          </div>
        ) : (
          <div className="px-1 py-1">
            ...
            <label className="mt-1 flex items-center justify-center gap-1 text-[9px] font-bold text-gray-500 cursor-pointer select-none whitespace-nowrap">
              <input type="checkbox" checked={performanceNotConducted} onChange={e=>handleChange('isPerformanceNotConducted', e.target.checked)} className="accent-amber-600"/>
              수행평가 미실시
            </label>
          </div>
        )}
      ...
      {renderPerformanceCell('perf1')}
      {renderPerformanceCell('perf2')}
      {renderPerformanceCell('perf3')}
      {renderPerformanceCell('perf4')}
      {renderPerformanceCell('perf5')}
...
export default function App() {
  ...
  const currentExamKey = `${vYear}|${vSem}|${vExam}`;
  ...
  if (scheduleToDisplay.length === 0) {
    ...
    if (currentExamKey === legacyKey && globalSettings.examSchedule?.length > 0) scheduleToDisplay = (globalSettings.examSchedule || []).filter(Boolean);
  }
  if (scheduleToDisplay.length === 0 && viewingScopes.length > 0) {
    ...
    scheduleToDisplay = Array.from(recoveredMap.values()).filter(Boolean).sort((a, b) => String(a?.date || '').localeCompare(String(b?.date || '')) || String(a?.grade || '').localeCompare(String(b?.grade || '')) || String(a?.period || '').localeCompare(String(b?.period || '')));
  }
  ...
  if (vExam === '수행평가') {
    ...
    let perfList = (globalSettings.perfSchedules?.[perfKey] || []).filter(Boolean);
    ...
    const formUniqueMap = new Map();
    scheduleToDisplay.forEach(item => { if(item) formUniqueMap.set(`${item.grade || ''}|${item.subject || ''}`, { grade: item.grade, subject: item.subject }); });
  ...
  const handleExportRatioCSV = () => {
    let csv = "\uFEFF학년,과목(학점),수행평가 실시 여부,1차 정기시험,2차 정기시험,수행평가(1),수행평가(2),수행평가(3),수행평가(4),수행평가(5),논술형 비율(%),계,최종확인\n";
    ...
    [1, 2, 3].forEach(g => {
      ...
      gradeRatios.forEach(r => {
        const row = [g, r.subject, r.isPerformanceNotConducted ? '미실시' : '실시', r.exam1, r.exam2, r.perf1, r.perf2, r.perf3, r.perf4, r.perf5, r.essay, r.total, r.isConfirmed ? r.confirmedBy : '미확인'];
        csv += row.map(escapeCSV).join(',') + '\n';
      });
    });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        ...
        <table className={`w-full border-collapse border-2 border-black text-center text-[15px] ${isPrintView ? 'print:text-[13px] print:w-full' : ''}`}>
          ...
          <tbody>
            {scheduleData.map((item, idx) => {
              ...
              const safeDate = String(item.date || '');
              const safeGrade = String(item.grade || '');
              const scopeId = getScopeId(y, s, e, item); 
  ...
  const renderRatioTableBlock = (dataRatios) => {
    return (
      <div className="w-full">
        {[1, 2, 3].map(g => {
          const gradeRatios = dataRatios.filter(r => String(r.grade) === String(g));
          ...
          if (gradeRatios.length === 0 && newRows.length === 0 && viewMode === 'status') return null;

          return (
            <div key={g} className="mb-8 animate-fade-in">
              <h3 className="text-xl font-black text-gray-800 mb-3 border-l-4 border-amber-500 pl-3 print:border-none print:pl-0">{g}학년</h3>
              {gradeRatios.length === 0 && newRows.length === 0 ? (
                <div className="text-center py-6 bg-gray-50 rounded-2xl text-gray-400 font-bold text-sm">데이터가 없습니다. 아래 버튼으로 과목을 추가하세요.</div>
              ) : (
                <div className="w-full overflow-x-auto bg-white shadow-sm border border-gray-200 print:shadow-none print:border-none print:overflow-visible">
                  <table className="w-full border-collapse border-2 border-black text-center text-[12px] print:text-[10px] table-fixed min-w-[800px] print:w-full">
                    <thead>
                      <tr>
                        <th rowSpan={2} className="border-2 border-black p-1 bg-gray-100 font-black w-[15%] print:w-36">과목(학점)</th>
                        <th colSpan={2} className="border-2 border-black p-1 bg-gray-100 font-black w-[16%] text-xs">정기시험(%)</th>
                        <th colSpan={5} className="border-2 border-black p-1 bg-gray-100 font-black w-[35%] text-xs">수행평가(차)</th>
                        <th rowSpan={2} className="border-2 border-black p-1 bg-gray-100 font-black w-[7%] text-[10px] leading-tight print:w-14">논술형<br/>비율(%)</th>
                        <th rowSpan={2} className="border-2 border-black p-1 bg-gray-100 font-black w-[7%] text-[10px] leading-tight print:w-12">계<br/>(100%)</th>
                        <th rowSpan={2} className="border-2 border-black p-1 bg-gray-100 font-black w-[12%] text-[10px] print:w-24">최종 확인</th>
                        {viewMode === 'ratio' && <th rowSpan={2} className="border-2 border-black p-1 bg-gray-100 font-black w-[8%] text-[10px] print:hidden">관리</th>}
                      </tr>
                      <tr>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-14">1차</th>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-14">2차</th>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-12">(1)</th>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-12">(2)</th>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-12">(3)</th>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-12">(4)</th>
                        <th className="border-2 border-black p-1 bg-gray-50 text-[10px] font-bold print:w-12">(5)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {gradeRatios.map((item, idx) => (
                        viewMode === 'status' ? (
                          <tr key={item.id || `gr_${idx}`}>
                            ...
                            {item.isPerformanceNotConducted ? (
                              <td colSpan={5} className="border border-black p-1 font-bold text-gray-500 bg-gray-50">미실시</td>
                            ) : (
                              <>
                            ...
                            <td className="border border-black p-1">{item.isConfirmed ? <span className="text-[10px] font-black text-emerald-700">{String(item.confirmedBy || '')} (완료)</span> : <span className="text-[10px] text-gray-400">미확인</span>}</td>
                          </tr>
                        ) : (
                          <RatioRow key={item.id || `gr_${idx}`} item={item} year={ratioYear} sem={ratioSem} grade={g} onSave={handleRatioSave} onDelete={handleRatioDelete} />
                        )
                      ))}
                      {viewMode === 'ratio' && newRows.map(item => <RatioRow key={item.id} item={item} year={ratioYear} sem={ratioSem} grade={g} onSave={handleRatioSave} onDelete={handleRatioDelete} />)}
                    </tbody>
                  </table>
                </div>
              )}
  ...
  const renderInputRatioTable = () => {
    ...
    return (
      <>
        ...
        {renderRatioTableBlock(ratiosToDisplay)}
      </>
    );
  };

  const renderStatusRatioTable = () => {
    const currentStatusRatios = (assessmentRatios || []).filter(r => String(r?.year) === String(vRatioYear) && String(r?.semester) === String(vRatioSem));
    let ratiosToDisplay = [];
    if (String(vRatioYear) === '2026' && String(vRatioSem) === '1') {
      ratiosToDisplay = defaultAssessment2026S1.map(def => {
      ...
      const toAdd = currentStatusRatios.filter(r => !ratiosToDisplay.some(d => d.subject === r.subject && String(d.grade) === String(r.grade)));
      ratiosToDisplay = [...ratiosToDisplay, ...toAdd];
    } else {
      ratiosToDisplay = [...currentStatusRatios];
    }
    
    if (ratiosToDisplay.length === 0) return <div className="p-8 text-center text-gray-500 font-bold bg-gray-50 rounded-2xl">해당 학기({vRatioYear}년 {vRatioSem}학기)에 입력된 평가 비율 데이터가 없습니다.</div>;
    return renderRatioTableBlock(ratiosToDisplay);
  };
        ...
        <main className="flex-1 flex flex-col items-center justify-start p-4 md:p-8 animate-fade-in relative z-0 print:p-0 w-full">
          ...
          {viewMode === 'scope' && (
            <div className="w-full max-w-5xl animate-fade-in mt-4">
              ...
              {renderScheduleTable(scheduleToDisplay, viewingScopes, vYear, vSem, vExam, false)}
            </div>
          )}
                        ...
                        <table className="w-full border-collapse border border-gray-300 text-center text-[12px] bg-white shadow-sm rounded-xl overflow-hidden">
                          ...
                          <tbody>
                            ...
                            <tr className="bg-amber-50 font-black">
                              <td colSpan={3} className="border border-black p-3 align-middle text-amber-900">학기말 최종 추정분할 점수</td>
                              <td className="border border-black p-2"><input type="number" step="0.01" value={cutoffScores.ab} onChange={e=>setCutoffScores({...cutoffScores, ab: e.target.value})} className="w-16 text-center text-[15px] font-black outline-none bg-white text-rose-600 focus:bg-amber-100 rounded px-1 py-1 border border-transparent focus:border-amber-400" /></td>
                              <td className="border border-black p-2"><input type="number" step="0.01" value={cutoffScores.bc} onChange={e=>setCutoffScores({...cutoffScores, bc: e.target.value})} className="w-16 text-center text-[15px] font-black outline-none bg-white text-rose-600 focus:bg-amber-100 rounded px-1 py-1 border border-transparent focus:border-amber-400" /></td>
                              <td className="border border-black p-2"><input type="number" step="0.01" value={cutoffScores.cd} onChange={e=>setCutoffScores({...cutoffScores, cd: e.target.value})} className="w-16 text-center text-[15px] font-black outline-none bg-white text-rose-600 focus:bg-amber-100 rounded px-1 py-1 border border-transparent focus:border-amber-400" /></td>
                              <td className="border border-black p-2"><input type="number" step="0.01" value={cutoffScores.de} onChange={e=>setCutoffScores({...cutoffScores, de: e.target.value})} className="w-16 text-center text-[15px] font-black outline-none bg-white text-rose-600 focus:bg-amber-100 rounded px-1 py-1 border border-transparent focus:border-amber-400" /></td>
                              <td className="border border-black p-2"><input type="number" step="0.01" value={cutoffScores.ei} onChange={e=>setCutoffScores({...cutoffScores, ei: e.target.value})} className="w-16 text-center text-[15px] font-black outline-none bg-white text-rose-600 focus:bg-amber-100 rounded px-1 py-1 border border-transparent focus:border-amber-400" /></td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
